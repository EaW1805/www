...
public class CommandersViewerPopup extends DraggablePanel {


    public CommandersViewerPopup(final List<CommanderDTO> commanders, final String title) {
        ...
        for (final CommanderDTO commander : commanders) {
            if (column == 0) {
                side1.add(new CommanderInfoPanel(commander));
                column = 1;
            } else {
                side2.add(new CommanderInfoPanel(commander));
                column = 0;
            }
        }
        ...
        (new DelEventHandlerAbstract() {
            ...
            public void execute(final MouseEvent event) {
                CommandersViewerPopup.this.open();
                imgX.deselect();
            }
