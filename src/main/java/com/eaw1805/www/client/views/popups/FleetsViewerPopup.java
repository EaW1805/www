...
public class FleetsViewerPopup extends DraggablePanel {

    public FleetsViewerPopup(final List<FleetDTO> fleets, final String title, final boolean includeZero) {
        ...
        for (FleetDTO fleet : fleets) {
            if (fleet.getFleetId() != 0 || includeZero) {
                if (column == 0) {
                    side1.add(new FleetInfoPanel(fleet, false));
                    column = 1;
                } else {
                    side2.add(new FleetInfoPanel(fleet, false));
                    column = 0;
                }
            }
        }
