...
/**
 * REST controller for showing another user's honour.
 */
...
public class UserHonourController
    ...
    @RequestMapping(method = RequestMethod.GET, value = "/user/{userName}/honour")
    public ModelAndView handleRequest(@PathVariable final String userName,
        ...
        if (profileUser == null) {
            ...
            final RedirectView redirectView = new RedirectView(request.getContextPath() + "/honour");
            redirectView.setExposeModelAttributes(false);
            return new ModelAndView(redirectView);
        }
        ...
        for (int db = ScenarioContextHolder.FIRST_SCENARIO; db <= ScenarioContextHolder.LAST_SCENARIO; db++) {
            ...
            watchedGames.addAll(watchGameManager.listByUser(profileUser));
        }
        ...
        final Profile vpsProfile = profileManager.getByOwnerKey(profileUser, ProfileConstants.VPS);
        int vps = 0;
        if (vpsProfile != null) {
        ...
        refData.put("userDateJoined", new Date(profileUser.getDateJoin() * 1000));
        ...
        final List<Achievement> lstAchievements = getAchievementManager().list(profileUser.getUserId());
        ...
        final int totPosts = forumPostsManager.getNumberOfPosts(profileUser);
        int postsLevel = 0;
        ...
        refData.put("recentAchievements", getAchievementManager().listRecentHonour(profileUser.getUserId(), 15));
        ...
        refData.put("profileStats", gameHelper.prepareUserStatistics(profileUser));

        LOGGER.info("[" + thisUser.getUsername() + "/" + thisUser.getRemoteAddress() + "] User/Honour " + profileUser.getUsername());
        return new ModelAndView("user/honourUser", refData);
    }
