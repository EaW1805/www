/*
 mooTree 2, rev.16 (compressed version)
...
var MooTreeControl = new Class({initialize:function (config, options) {
...
var MooTreeNode = new Class({initialize:function (options) {
    ...
    this.div = {main:new Element('div').addClass('mooTree_node'), indent:new Element('div'), gadget:new Element('div'), icon:new Element('div'), text:new Element('div').addClass('mooTree_text'), sub:new Element('div')}
    ...
    var f = function () {
        new XHR({method:'GET', onSuccess:this._loaded.bind(this), onFailure:this._load_err.bind(this)}).send(url, vars || '');
    };
