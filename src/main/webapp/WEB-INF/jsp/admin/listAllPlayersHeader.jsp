...
<jsp:useBean id="usersList" scope="request" type="org.springframework.beans.support.PagedListHolder"/>
...
<script type="text/javascript">
    ...
    function goToPage() {
        ...
        if (page !== null && !isNaN(page) && page == Math.floor(page) && page > 0 && page <= ${usersList.pageCount}) {
            page--;
            document.location.href = '<c:url value="/users/list?page="/>' + page;
        }
        $.unblockUI();
    }
...
<div class="player" style="height: 200px;">
    ...
    <h4 class="pagination">
        ${usersList.nrOfElements} Users • <a href="javascript:openGoToPageForm()">Page ${usersList.page + 1 }
        of ${usersList.pageCount} </a> •

        <c:forEach items="<%=getNavigationMenu(usersList)%>" var="thisPage">

            <c:if test="${thisPage == usersList.page  }">
                <a class="current" href='<c:url value="/users/list?page=${thisPage}"/>'> ${thisPage+1}</a>
            </c:if>
            <c:if test="${thisPage != usersList.page  }">
                <c:if test="${thisPage != -1 }">
                    <a class="others" href='<c:url value="/users/list?page=${thisPage}"/>'> ${thisPage+1}</a>
                </c:if>
                <c:if test="${thisPage == -1 }">
